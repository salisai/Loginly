...
const Register = () => {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white/[0.04] backdrop-blur-xl border border-[var(--color-border)] rounded-2xl p-8 md:p-10 shadow-2xl relative overflow-hidden">
      <div className="absolute top-0 right-0 w-40 h-40 bg-purple-500/10 rounded-full blur-[100px] pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-0 left-0 w-40 h-40 bg-green-500/10 rounded-full blur-[100px] pointer-events-none" aria-hidden="true" />

      <div className="relative z-10">
        <h2 className="text-heading-1 text-center mb-2 text-white">Create Account</h2>
        <p className="text-caption text-center mb-8 text-[var(--text-body)]">Join us and start your journey</p>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="username" className="block text-caption font-medium text-[var(--text-body)] mb-2">
              Name
            </label>
            <input
              type="text"
              ...
              className={`block w-full px-4 py-3.5 text-body bg-black/20 border rounded-xl text-white placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/50 focus:border-[var(--color-primary)] transition-all duration-200 ${errors.username ? "border-red-500/50 focus:border-red-500 focus:ring-red-500/20" : "border-[var(--color-border)]"}`}
            />
            {errors.username && <p className="mt-1.5 text-caption text-red-400 font-medium">{errors.username}</p>}
          </div>
        ...
        <div className="mt-8">
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="px-3 bg-[var(--color-surface)] text-[var(--text-muted)] uppercase tracking-widest font-semibold text-caption">Or register with</span>
            </div>
          </div>
